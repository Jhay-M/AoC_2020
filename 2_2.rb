...
grouped_input = input.each_slice(3).to_a.each do |arr|
  arr.map! { |x| x.tr(':', '') }
end

count = 0
i = 0
while i < grouped_input.size
  index = grouped_input[i][0].split('-').map(&:to_i).map { |x| x - 1 }
  if grouped_input[i][2][index[0]] == grouped_input[i][1] && grouped_input[i][2][index[1]] == grouped_input[i][1]
    nil
  elsif grouped_input[i][2][index[0]] == grouped_input[i][1] || grouped_input[i][2][index[1]] == grouped_input[i][1]
    count += 1
  end
